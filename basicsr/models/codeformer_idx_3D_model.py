...
class CodeFormerIdx3DModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        self.warmup = train_opt.get("warmup", 5000)
        self.net_g.train()
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.generate_idx_gt:
            ...
            _, _, _, quant_stats = self.hq_vqgan_fix.quantize(x)
            min_encoding_indices = quant_stats["min_encoding_indices_s"]
            self.idx_gt_s = min_encoding_indices  # .view(self.b, -1)
            min_encoding_indices2 = quant_stats["min_encoding_indices_t"]
            self.idx_gt_t = min_encoding_indices2  # .view(self.b, -1)

        logits_s, logits_t, lq_feat = self.net_g(self.input, w=0, code_only=True)

        if self.hq_feat_loss:
            # quant_feats
            quant_feat_gt = self.net_g.quantize.get_codebook_feat(
                self.idx_gt_s,
                self.idx_gt_t,
                shape=[
                    self.b,
        ...
        # cross_entropy_loss
        if self.cross_entropy_loss and current_iter > self.warmup:
            # (b)(thw)n -> (b)n(thw)
            cross_entropy_loss_s = (
                F.cross_entropy(logits_s.permute(0, 2, 1), self.idx_gt_s)
                * self.entropy_loss_weight
            )
            l_g_total += cross_entropy_loss_s
            loss_dict["cross_entropy_loss_s"] = cross_entropy_loss_s

            cross_entropy_loss_t = (
                F.cross_entropy(logits_t.permute(0, 2, 1), self.idx_gt_t)
                * self.entropy_loss_weight
            )
            l_g_total += cross_entropy_loss_t
            loss_dict["cross_entropy_loss_t"] = cross_entropy_loss_t
    ...
    def test(self):
        with torch.no_grad():
            if hasattr(self, "net_g_ema"):
                self.net_g_ema.eval()
                self.output, _, _, _ = self.net_g_ema(self.input, w=0)
            else:
                logger = get_root_logger()
                logger.warning("Do not have self.net_g_ema, use self.net_g.")
                self.net_g.eval()
                self.output, _, _, _ = self.net_g(self.input, w=0)
                self.net_g.train()
